...
def lambda_handler(event, context):
    ...
    try:
        ...
        # Find the training job by replicate_id
        training_jobs_table = dynamodb.Table(TRAINING_JOBS_TABLE_NAME)
        
        # Scan for job with this replicate_id (since it's not the primary key)
        ...
        if not jobs:
            print(f"No training job found for Replicate ID: {prediction_id}")
            return {
                'statusCode': 404,
                'headers': {'Content-Type': 'application/json'},
        ...
        training_jobs_table.update_item(
            Key={'job_id': job_id},
